...
let standard_formula x y =
   (y - 6) * 5 + x - 1 |> handle_ij |> int_to_upper |> letter_to_char

let rotated_formula x y =
   (x - 1) * 5 + y - 6 |> handle_ij |> int_to_upper |> letter_to_char
...
let _run formula input =
   input |> Utils.explode |> string_to_pairs |> List.map (search formula) |> List.to_seq |> String.of_seq
